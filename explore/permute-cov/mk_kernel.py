    ...
    def forward(self, x1, x2, **params):
        ...
        for t1 in range(self.num_tasks):
            for t2 in range(self.num_tasks):
                inder += 1
                if t1 == t2:
                    mat_list[inder] = self.sq_exp_fun(x1, x2, self.log_task_lengthscales[t1], **params)
                else:
                    mat_list[inder] = self.sq_exp_mix(x1, x2, self.log_task_lengthscales[t1], self.log_task_lengthscales[t2], **params)
