...
model.covar_module.in_task1.log_lengthscale.data[0][0][0] = math.log(1)
model.covar_module.in_task2.log_lengthscale.data[0][0][0] = math.log(0.3)

#print(model.covar_module.output_scale_kernel.covar_factor.data[0])
...
model.covar_module.output_scale_kernel.covar_factor.data[0] = torch.tensor([[0,0],[0,0]])
model.covar_module.output_scale_kernel.log_var.data[0][0] = math.log(50)
model.covar_module.output_scale_kernel.log_var.data[0][1] = math.log(20)
...
plt.plot(domain_x.detach().numpy(),sample[0,:,0].detach().numpy()+sample[0,:,1].detach().numpy())
plt.legend(['Task 1','Task 2'])
plt.show()

x = domain_x.detach().numpy()
y1 = sample[0,:,0].detach().numpy()
y2 = sample[0,:,0].detach().numpy() + sample[0,:,1].detach().numpy()
data = np.column_stack([x,y1,y2])
np.savetxt("example5.csv",data,delimiter=',')
print(model.covar_module.output_scale_kernel.covar_matrix.evaluate())
